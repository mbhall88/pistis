...
def test_command_line_interface():
    ...
    assert "Either --fastq, --bam or both must be given as arguments." in result.output
    ...
    assert 'Path "{}" does not exist.'.format(bad_fastq) in bad_fastq_result.output

    # test cli with bad bam path passed
    bad_bam = "../../bad.bam"
    bad_bam_result = runner.invoke(pistis.main, ["--bam", bad_bam])
    assert bad_bam_result.exit_code == 2
    assert 'Path "{}" does not exist.'.format(bad_bam) in bad_bam_result.output
    ...
    assert (
        "invalid choice: hownowbrowncow. (choose from kde, scatter, hex)"
